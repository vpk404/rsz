...
def get_k_brute_force_setting() -> int:
    while True:
        s = input("Enable K-value brute force at the end? Enter K limit (0 to disable, e.g. 10000): ").strip()
        if not s:
            return 0
        try:
            v = int(s)
...
# ==============================================================================
# RECOVERY FUNCTIONS (With RIPEMD Fix & Improved Math)
# ==============================================================================
...
def modinv(a, m=N):
    a = a % m
    if a == 0: raise ValueError("gcd(a, m) != 1")
    return pow(a, -1, m)
...
# ==============================================================================
# MAIN RECOVERY EXECUTION ENGINE
# ==============================================================================
    ...
    while found_something:
        ...
        for group in parsed_groups:
            ...
            if master_params:
                ...
                for s_t_hex, z_t_hex, pub_target in txs:
...
def main():
    global TOTAL_ADDRESSES, MAX_TRANSACTIONS, BRUTE_MAX_K
    try:
        ...
        # Fallback to run recovery if it was globally set, else 0
        run_recovery(OUTPUT_R_NON, brute_max_k=BRUTE_MAX_K)
        sys.exit(0)
