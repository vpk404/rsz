...
def solve_hnp(signatures, pubkey_hex):
    ...
    for B in biases:
        ...
        for i in range(m):
            M[m+1, i] = K_val * us[i]
        M[m+1, m+1] = 1 # Force affine part

        print("\n" + "="*60)
        print(f"   LATTICE MATRIX CONSTRUCTED (Dim: {dim}x{dim})")
        print("="*60)

        # Display Matrix preview (Professional output)
        print("Matrix Structure (Preview):")
        print("-" * 60)

        # Row 0 (Example of Diagonal K*n)
        kn_str = f"{K_val * n:.1e}"
        print(f"Row 0   : [ {kn_str}, 0, ..., 0, 0 ]")
        print("...")

        # Row m (t coefficients)
        t_str = f"{K_val * ts[0]:.1e}"
        print(f"Row {m:<4}: [ {t_str}, ..., 1, 0 ]")

        # Row m+1 (u coefficients)
        u_str = f"{K_val * us[0]:.1e}"
        print(f"Row {m+1:<4}: [ {u_str}, ..., 0, 1 ]")
        print("-" * 60)

        print(f"[*] Executing LLL Reduction on {dim}x{dim} Matrix...")
        print("    This may take a moment depending on CPU...")

        # Run LLL
        try:
            # We can use verbose=True in some Sage versions, or just wrapper
            M_red = M.LLL()

            print("[+] LLL Reduction Complete.")
            print(f"    Shortest Vector Norm: {float(M_red[0].norm()):.2e}")
